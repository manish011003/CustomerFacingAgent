...
6. **Architecture** — Next.js customer UI + FastAPI + policy engine + issue router (disruption / assist / help) + Elasticsearch/JSON KB. LLM optional. Diagram from [docs/system-architecture.md](system-architecture.md).
...
9. **Scenario 3 Meher + operations** — 6h: delayed-hours hotel, not full night. ₹2,000 > ₹1,500 → supervisor. Dashboard shows transcript, policy, actions, escalation reason.
10. **Limits and tools** — Data pack only. Simulated actions. Inputs/sources/assumptions. Cursor used for build; policy tests prove the model does not own decisions.

Speaker note for every scenario slide: show the inline confirmation or escalation card, then the operations case, not a CRM profile.
